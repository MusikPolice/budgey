...
public class AccountServiceTest {
	...
	public void insertRetrieveTest() {
		try (AutoCommittingHandle handle = new AutoCommittingHandle(dbi)) {
			final int numStartingAccounts = service.getAccountsWithTransactions(handle).size();
			...
			final Account account = Account.newBuilder(accountNumber)
			                               .setType(type)
			                               .setBalance(balance)
			...
			service.insertAccountWithTransactions(handle, account);
			final List<Account> accounts = service.getAccountsWithTransactions(handle);
